...
1. get_crypto_market_data: теперь мы запрашиваем сразу 1000 криптовалют, отсортированных по рыночной капитализации, что 
   упрощает деление на группы.
2. get_crypto_price: через GET запрос к API сайта CoinGecko.com получает цену нужной криптовалюты
3. update_crypto_list: обновляет второй Combobox с криптовалютами в выбранной группе, а также сохраняет идентификаторы 
   и значения рыночной капитализации.
4. update_crypto_price_and_market_cap: показывает курс и рыночную капитализацию выбранной криптовалюты, обновляя метки 
   price_label и market_cap_label.
